...
# by setting global variables. You would never use global variable
# in production server. 
$home = {}

# This is a ruby class that includes validatation from ActiveRecord.
...
class Home
  ...
  # ActiveModel that provides validataions.
...
# error code (https://developer.mozilla.org/en-US/docs/Web/HTTP/Status)
class TerraTownsMockServer < Sinatra::Base
  ...
  def x_user_uuid
    return 'f39cf958-47a2-40c7-96cb-e8479413d33d'
  end
  ...
  def find_user_by_bearer_token
   ...
   # if we can find it than return an error or it doesn't match 
   # code = access_code = token
    code = auth_header.split("Bearer ")[1]
    if code != x_access_code
      error 401, "a100 1 Failed to authenicate, bearer token invalid and/or teacherseat_user_uuid invalid"
    end
  ...
  post '/api/u/:user_uuid/homes' do
    ...
    # puts will print to the terminal simlar to print or console.log
    puts "# create - POST /api/homes"

    #begin/resurce s a try/catch, if an error occurs, result it.
    begin
    ...
    # make easier to work with the code
    name = payload["name"]
    ...
    # ensure our vaildation checks pass otherwise
    # return the errors  
    unless home.valid?
